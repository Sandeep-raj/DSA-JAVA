...
public class GasStations {
    ...
    public static double place(int[] gasStations, int addStations) {
        // Brute Force
        // int[] addedStations = new int[gasStations.length - 1];

        // for (int station = 0; station < addStations; station++) {
        // double maxStationDistance = 0;
        // int maxStationIdx = -1;

        // for (int currStation = 0; currStation < gasStations.length - 1;
        // currStation++) {
        // double currDistance = (double)(gasStations[currStation + 1] -
        // gasStations[currStation])/(double)(addedStations[currStation] + 1);
        // if (currDistance > maxStationDistance) {
        // maxStationDistance = currDistance;
        // maxStationIdx = currStation;
        // }
        // }

        // addedStations[maxStationIdx]++;
        // }
        ...
        for(int i =0; i < gasStations.length - 1; i++) {
            pq.add(new Pair(gasStations[i+1] - gasStations[i], i));
        }

        for (int station = 0; station < addStations; station++) {
            Pair top = pq.peek();
            addedStations[top.second]++;
            double newDist = ((double)top.first/(double)(addedStations[top.second]+1));

            pq.remove(top);
            pq.add(new Pair(newDist, top.second));
        }
        ...
        // double mid = (double)(start+end)/2.0;

        // int cntStations = numOfGasStations(gasStations, mid);
        // if(cntStations > addStations) {
        // start = mid;
        // }else {
        // end = mid;
        // }
    ...
    static int numOfGasStations(int[] arr, double dist) {
        int countGasStations = 0;
        for (int i = 0; i < arr.length - 1; i++) {
            int gasStations = (int) ((arr[i + 1] - arr[i]) / dist);
            if (arr[i + 1] - arr[i] == (dist * gasStations)) {
                gasStations--;
            }
            countGasStations += gasStations;
        }

        return countGasStations;
    }
}

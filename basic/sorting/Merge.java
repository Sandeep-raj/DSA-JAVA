...
public class Merge {
    public static int[] sort(int[] arr) {
        int[] result = sortRec(arr, 0, arr.length - 1);
        System.out.println(Arrays.toString(result));
        return result;
    }

    static int[] sortRec(int[] arr, int start, int end) {
        if(start == end) {
        ...
        int[] leftArr = sortRec(arr, start, mid);
        int[] rightArr = sortRec(arr, mid + 1, end);

        return insertionSort(leftArr, rightArr);
    }

    static int[] insertionSort(int[] leftArr, int[] rightArr) {
        int ptr1 = 0;
        int ptr2 = 0;
        int cnt = 0;
        ...
        while (ptr1 < leftArr.length && ptr2 < rightArr.length) {
            if (leftArr[ptr1] < rightArr[ptr2]) {
                result[cnt] = leftArr[ptr1];
                ptr1++;
            }else {
                result[cnt] = rightArr[ptr2];
                ptr2++;
            }
            cnt++;
        }

        while (ptr1 < leftArr.length) {
            result[cnt] = leftArr[ptr1];
            ptr1++;
            cnt++;
        }

        while (ptr2 < rightArr.length) {
            result[cnt] = rightArr[ptr2];
            ptr2++;
            cnt++;
        }
